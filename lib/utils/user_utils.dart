...
class UserUtils {
  ...
  static Future<Map<String, dynamic>?> getUserProfile() async {
    try {
      ...
      print(
          'UserUtils: Stack trace: ${StackTrace.current}'); // Debug stack trace
      return null;
    }
  }
  ...
  static Future<String> getUserName() async {
    try {
      ...
      if (!isLoggedIn()) {
        print(
            'UserUtils: Usuário não logado, retornando nome padrão'); // Debug log
        return 'Usuário';
      }
      ...
      try {
        ...
        if (profile != null &&
          ...
          print(
              'UserUtils: Nome encontrado no Firestore: ${profile['name']}'); // Debug log
          return profile['name'].toString();
        }
      } catch (e) {
        print(
            'UserUtils: Erro ao buscar no Firestore, usando Authentication: $e'); // Debug log
      }

      // Fallback para Authentication
      final displayName = getUserDisplayName();
      print(
          'UserUtils: Usando nome do Authentication: $displayName'); // Debug log
      return displayName;
    } catch (e) {
